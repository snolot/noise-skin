...
const Particles = (renderer, scene, camera, skinnedMesh, _options) => {
	...
	const computePosShader =  `
		...
		void main() {
		    ...
		    if(uActive == 0){
		    	pos = texture2D(textureOriginalPosition, uv).rgb;
			}

		    pos += vel * uSpeedOffset * float(uActive);
	...
	const initVelocityTexture = texture => {
		...
		for(let i=0; i<AMOUNT; i++){
			...
			data[i * 4 + 3] = Math.random(); // used as life
		}
	...
	const initGpuCompute = () => {
    	...
    	posUniforms.uActive = {type:'i', value:0}
	...
	const initGeometry = () => {
		...
		for(let i = 0; i < AMOUNT; i++) {
			actives[i] = 1.0;//(Math.random() > .89) ? 0.0 : 1.0;
		...
		material.onBeforeCompile = (shader) => {
			...
			shader.uniforms.uActive =  { type:'i', value:0 }
			shader.uniforms.uScale =  { type:'f', value:0 }
			...
			shader.vertexShader = shader.vertexShader.replace(
				...
				vec3 finalPos      = pos;//mix(oldPos, pos, .75);
	...
	const show = () => {
		...
		new TWEEN.Tween(obj).to({value:1.0},  3000).
		onUpdate(() => {
			_shader.uniforms.uScale.value = obj.value;
			_shader2.uniforms.uScale.value = obj.value;
		}).delay(3000).start()
	};

	const activate = () => {
		//setTimeout(() => {
			posUniforms.uActive.value = 1;
			_shader.uniforms.uActive.value = 1;
			_shader2.uniforms.uActive.value = 1;
			console.log(posUniforms.uActive.value);
			const obj = {
				value:0
			}
			new TWEEN.Tween(obj).to({value:.5},  3000).
			onUpdate(() => {
				posUniforms.uSpeedOffset.value = obj.value;
			}).start()
		//},  10000);
	}
